...
public class ParkingLotGUI extends JFrame {
    ...
    private void loadFromDB() {
        for (StopOver stop : JSONReader.load(DB_PATH)) {
            lot.loadStop(stop);
            if (stop.isOngoing()) {
                Vehicle v = stop.getVehicle();
                slotToVehicle.put(stop.getPosition(), v);
                plateToVehicle.put(v.getPlate(), v);
                markOccupied(stop.getPosition(), v);
            }
        }
        refreshInfo();
    }
    ...
    private void saveDB() {
        JSONWriter.save(DB_PATH, lot.getStops());
    }
